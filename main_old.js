...
class basicObj {
    ...
    drawObject(textureIndex) {
        ...
        gl.uniform2f(this.uniformLocations.size, this.size[0], this.size[1]);
...
function generateGrassPoints() {
    ...
    for (let i = 0; i < GRASS_AMOUNT; i++) {
        ...
        if (Math.random() < FLOWER_FRACTION) {
            grassdata[i*4+ 2] = Math.floor(getRandomInRange(0,  4));
        } else {
            grassdata[i*4+ 2] = Math.floor(getRandomInRange(4,  12));
        }
        grassdata[i*4+ 3] = 0;   
    }  
}
...
async function loadGrassTexture(grassUnifLocation) {
    ...
    gl.texImage3D( gl.TEXTURE_2D_ARRAY, 0, gl.RGBA, 
        16, 16, 12, 0, gl.RGBA, gl.UNSIGNED_BYTE, image);
...
async function main() {
    ...
    let staticObjs = [
        new basicObj([100,20],[64, 64], objs.basicObj.uniformLocations),
        new basicObj([100,130],[64, 64], objs.basicObj.uniformLocations),
        new basicObj([250,90],[64, 64], objs.basicObj.uniformLocations),
        new basicObj([300,200],[64, 64], objs.basicObj.uniformLocations),
    ];
    ...
    function drawFrame() {
        ...
        for (let i=mouse_frame_index; i < GRASS_AMOUNT; i+=MOUSE_UPDATE_RATE) {
            ...
            grassdata[i*4+3] = Math.max((dist < MOUSE_RANGE_SQUARED)*1000, grassdata[i*4+3]) - LIVING_DECAY;//- *dt
        }
